...
//! 実行時の更新確認は共有フォルダ 更新置き場\Tachimi\ だけを見る。信頼基盤は minisign 署名。
//! 更新先・公開鍵はアドレス直書き（割符外付けは終盤フェーズで導入予定）。最終ゲートは minisign 検証。

use serde::Serialize;
use std::path::PathBuf;

const APP_UPDATE_SUBDIR: &str = "Tachimi";
/// 更新置き場（共有ドライブ）。
const UPDATE_LOCAL_DIR: &str =
    r"G:\共有ドライブ\CLLENN\編集部フォルダ\編集企画部\編集企画_C班(AT業務推進)\DTP制作部\App_installer";
/// Tachimi updater 公開鍵（minisign・二重base64）。秘密鍵は鍵フォルダのみ。
const UPDATER_PUBKEY: &str = "dW50cnVzdGVkIGNvbW1lbnQ6IG1pbmlzaWduIHB1YmxpYyBrZXk6IDE2NjcxRTg5NEQ4NjE4QzQKUldURUdJWk5pUjVuRmlUS1lTblVodVlCK1huUzg2TFdTZTUrMEhVdXVZRlIzYmo5akxQblViS0oK";
...
/// 更新フォルダ解決: 更新置き場\Tachimi 優先、無ければ直下（移行用）。
fn resolve_update_dir() -> Result<Option<PathBuf>, String> {
    let base_dir = match crate::security::ensure_directory_read_path(UPDATE_LOCAL_DIR) {
        Ok(d) => d,
        Err(_) => return Ok(None), // 更新置き場(G:)未接続なら更新なし扱い
    };
...
/// Tauri の .sig は二重base64。read → base64 decode → Signature::decode → verify。
...
pub async fn apply_local_update(app: tauri::AppHandle, setup_path: String) -> Result<(), String> {
    ...
    let src = crate::security::ensure_read_path(&info.setup_path)?;

    let temp = crate::security::app_temp_dir();
