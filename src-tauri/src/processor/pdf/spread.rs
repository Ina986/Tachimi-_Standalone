...
pub fn generate_spread_pdf(
    ...
    while (file_idx as usize) < total || (add_white_page && file_idx == -1) {
        ...
        // 左ページ（存在すれば）
        let (pdf_left, left_width_mm, left_height_mm) =
            if left_file_idx >= 0 && (left_file_idx as usize) < total {
        ...
        if let Some(ref font) = nombre_font {
            ...
            if file_idx == -1 {
                ...
                let right_text_x = right_x + right_width_mm / 2.0
                    - (right_num_str.len() as f32 * nombre_font_size_pt * 0.3 / 2.0);
                current_layer.use_text(
                    &right_num_str,
                    nombre_font_size_pt,
                    Mm(right_text_x),
